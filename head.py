...
from .block import DFL, SAVPE, BNContrastiveHead, ContrastiveHead, Proto, Residual, SwiGLUFFN, DenseSemanticGenerator
...
class GatedFusion(nn.Module):
    ...
    def __init__(self, c_p2, c_p3):
        super().__init__()
        self.p2_down = nn.Sequential(
            nn.Conv2d(c_p2, c_p3, 3, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(c_p3),
            nn.SiLU()
        )
        self.gate = nn.Sequential(
            nn.Conv2d(c_p3 * 2, c_p3, 1, bias=False),
            nn.BatchNorm2d(c_p3),
            nn.Sigmoid()
        )
        self.final_conv = nn.Sequential(
            nn.Conv2d(c_p3, c_p3, 3, padding=1, bias=False),
            nn.BatchNorm2d(c_p3),
            nn.SiLU()
        )

    def forward(self, p3, p2):
        p2_aligned = p2
        concat_feat = torch.cat([p3, p2_aligned], dim=1)
        gate_weight = self.gate(concat_feat)
        fused = p3 + p2_aligned * gate_weight
...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        self.sem_generator = DenseSemanticGenerator(in_channels=c_p2)
        self.p2_to_p3_conv = nn.Sequential(
            nn.Conv2d(c_p2, c_p3, 1, stride=1, bias=False),
            nn.BatchNorm2d(c_p3),
            nn.SiLU()
        )
        self.fusion_conv = nn.Sequential(
            nn.Conv2d(c_p3 * 2, c_p3, 1, stride=1, bias=False),
            nn.BatchNorm2d(c_p3),
            nn.SiLU(),
            nn.Conv2d(c_p3, c_p3, 3, padding=1, groups=c_p3, bias=False),
            nn.BatchNorm2d(c_p3),
            nn.SiLU()
        )
        self.res_conv = nn.Conv2d(c_p3, c_p3, 1, bias=False) if c_p3 != c_p3 else nn.Identity()

        c2, c3 = max((16, c_detect[0] // 4, self.reg_max * 4)), max(c_detect[0], min(self.nc, 100))

        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(x, c2, 3), Conv(c2, c2, 3), nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in c_detect
        )
        self.cv3 = nn.ModuleList(
            nn.Sequential(Conv(x, c3, 3), Conv(c3, c3, 3), nn.Conv2d(c3, self.nc, 1)) for x in c_detect
        )
        self.dfl = DFL(self.reg_max) if self.reg_max > 1 else nn.Identity()
        self.end2end = False
        self.cross_fusion = GatedFusion(c_p2=c_p2, c_p3=c_p3)

    def forward(self, x):
        feat_p2 = x[0]
        feats_detect = x[1:]
        _, sem_prob_p2 = self.sem_generator(feat_p2)
        feat_p3 = feats_detect[0]
        feats_detect[0] = self.cross_fusion(feat_p3, feat_p2)
        aux_data = [sem_prob_p2]
        x = feats_detect
    ...
    def bias_init(self):
        ...
        # x = m.y if m.y is not None else m.x
